...
public class JwtTokenUtil implements Serializable {
    ...
    public String generateToken(UserDetails userDetails, boolean rememberMe) {
        ...
        Date expiration = new Date(System.currentTimeMillis() + Constant.JWT_TOKEN_VALIDITY_WITHOUT_REMEMEBERME);
        
        if(rememberMe) {
        	expiration = new Date(System.currentTimeMillis() + Constant.JWT_TOKEN_VALIDITY_WITH_REMEMBERME); 
        }
	...
	private String doGenerateToken(Map<String, Object> claims, String subject, Date expiration) {
		...
		return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis()))
				.setExpiration(expiration).signWith(SignatureAlgorithm.HS512, JWT_KEY).compact();
	}
